...
const fakeCredentials = {
  email: 'db2TesterFake@gmail.com',
  password: '12345678a',
};

describe('User auth', () => {
  it('should register user on /api/accounts/sign-up', async () => {
    await TestUtil.checkPostingResource(
      {
        url: '/api/accounts/sign-up',
        method: 'post',
        body: credentialForRegister,
      },
      [
        {
          name: 'firstName',
  ...
  it('should auth user on /accounts/sign-in', async () => {
    await TestUtil.checkPostingResource(
      {
  ...
  it('should recover password on /api/accounts/forgot-password', async () => {
    await TestUtil.checkPostingResource(
      {
  ...
  after(async () => {
    const [user] = await queries.getUserByEmail(credentialForRegister.email);
    await queries.deleteTokenByUserId(user.id);
    await queries.deleteUserByEmail(credentialForRegister.email);
    await deleteAllFileFromFolder('User', user.id);
  });
});

describe('User auth (Negative tests)', () => {
  before(async () => {
    const { confirmPassword, photo, ...rest } = credentialForRegister;
    await queries.insertNewUser({
      ...rest,
      photo: uriPhoto,
    });
  });
  it('should produce error (such user already exists) on /api/accounts/sign-up', async () => {
    it('should register user on /api/accounts/sign-up', async () => {
      await TestUtil.checkPostingResource(
        {
          url: '/api/accounts/sign-up',
  ...
  it('should produce error (no such user) on /accounts/sign-in', async () => {
    await TestUtil.checkPostingResource(
      {
        url: '/api/accounts/sign-in',
        body: fakeCredentials,
      },
      [
        {
          name: 'success',
  ...
  it('should produce error (no such user) on /api/accounts/forgotPassword', async () => {
    await TestUtil.checkPostingResource(
      {
        url: '/api/accounts/forgotPassword',
        body: { email: fakeCredentials.email },
      },
      [
        {
          name: 'success',
